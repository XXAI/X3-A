...
* @created    2015-07-20
*
* Controlador `Inventario`: Manejo de usuarios del sistema
*
*/
class InventarioController extends Controller {
	...
	public function index(){
		...
		if(array_key_exists("pagina", $datos)){
			...
			if(array_key_exists("buscar", $datos)){
				...
				$valor   = $datos["valor"];
				$data = Inventario::with("InventarioMetadato", "Almacen", "Articulo")->orderBy($order, $orden);
				...
				$data = Inventario::with("InventarioMetadato", "Almacen", "Articulo")->skip($pagina-1)->take($datos["limite"])->orderBy($order, $orden)->get();
				$total =  Inventario::all();
			}
			
		}
		else{
			$data = Inventario::with("InventarioMetadato", "Almacen", "Articulo")->get();
			$total = $data;
		}
	...
	public function campos($datos, $data){
        ...
        if ($data->save()) { 
            ...
            if(property_exists($datos, "inventario_metadato")){
                ...
                InventarioMetadato::where("servidor_id", $servidor_id)->where("inventario_id", $data->id)->delete();
                ...
                foreach ($inventario_metadato as $key => $value) {
                    ...
                    if($value != null){
                        ...
                        if($value->valor != ""){
	                        ...
	                        $item = InventarioMetadato::where("servidor_id", $servidor_id)->where("inventario_id", $data->id)->where("metadatos_id", $value->metadatos_id)->first();
	                        //si no existe crear
	                        if(!$item)
	                            $item = new InventarioMetadato;
	...
	public function show($id){
		$data = Inventario::with("InventarioMetadato", "Almacen", "Articulo")->find($id);			

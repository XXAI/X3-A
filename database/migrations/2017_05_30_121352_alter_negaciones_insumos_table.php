...
class AlterNegacionesInsumosTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('negaciones_insumos', function (Blueprint $table) {
                        $table->timestamp('ultima_entrada')->nullable()->change();
                        $table->integer('cantidad_acumulada_unidosis', 11)->after('cantidad_acumulada');
                        $table->integer('cantidad_entrada_unidosis', 11)->after('cantidad_entrada');
        });
    }
